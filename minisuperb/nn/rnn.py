...
from minisuperb.nn.interface import AbsFrameModel

__all__ = ["RNNEncoder", "SuperbDiarizationModel", "RNNLayer"]


def downsample(x: torch.Tensor, x_len: torch.LongTensor, sample_rate: int,
               sample_style: str) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if sample_style == "drop":
        ...
        raise NotImplementedError(
            f"Sample style={sample_style} not supported.")
...
class RNNLayer(nn.Module):
    ...
    def forward(self, xs: torch.Tensor, xs_len: torch.LongTensor):
        ...
        if self.sample_rate > 1:
            output, xs_len = downsample(output, xs_len, self.sample_rate,
                                        self.sample_style)
...
class SuperbDiarizationModel(AbsFrameModel):
    ...
    def __init__(self, input_size: int, output_size: int, rnn_layers: int,
                 hidden_size: int):
        super().__init__()
        self._input_size = input_size
        self._output_size = output_size
