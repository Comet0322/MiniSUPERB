...
class ConvBlock(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, residual, dropout, batch_norm,
                 activate):
...
class CnnNpc(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_blocks):
            ...
            # ConvBlock
            self.blocks.append(
                ConvBlock(h_dim, hidden_size, res, dropout, batch_norm,
                          activate))
            ...
            if self.disable_cross_layer and (i != (n_blocks - 1)):
                ...
                self.masked_convs.append(
                    MaskConvBlock(hidden_size, hidden_size, kernel_size,
                                  cur_mask_size))
    ...
    def create_msg(self):
        msg_list = []
        msg_list.append(
            "Model spec.| Method = NPC\t| # of Blocks = {}\t".format(
                self.n_blocks))
        msg_list.append("           | Desired input mask size = {}".format(
            self.input_mask_size))
        msg_list.append(
            "           | Receptive field size = {}".format(self.kernel_size +
                                                            2 * self.n_blocks))
        return msg_list
    ...
    def report_ppx(self):
        ...
        if self.apply_vq:
            ...
            rt = [vq_layer.report_ppx()
                  for vq_layer in self.vq_layers] + [None]
            return rt[0], rt[1]
        else:
            return None, None
