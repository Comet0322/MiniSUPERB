...
class TransformerConfig(object):
    ...
    def __init__(
        self,
        hidden_size:
        int = 768,  # Size of the encoder layers and the pooler layer.
        num_hidden_layers:
        int = 3,  # Number of hidden layers in the Transformer encoder.
        num_attention_heads:
        int = 12,  # Number of attention heads for each attention layer in the Transformer encoder.
        intermediate_size:
        int = 3072,  # The size of the "intermediate" (i.e., feed-forward) layer in the Transformer encoder.
        hidden_act:
        str = "gelu",  # The non-linear activation function (function or string) in the encoder and pooler. If string, "gelu", "relu" and "swish" are supported.
        hidden_dropout_prob:
        float = 0.1,  # The dropout probabilitiy for all fully connected layers in the embeddings, encoder, and pooler.
        attention_probs_dropout_prob:
        float = 0.1,  # The dropout ratio for the attention probabilities.
        initializer_range:
        float = 0.02,  # The sttdev of the truncated_normal_initializer for initializing all weight matrices.
        layer_norm_eps: float = 1.0e-12,  # The epsilon used by LayerNorm.
        share_layer: bool = False,  # Share layer weights
        pre_layer_norm:
        bool = False,  # To apply the pre layer normalization technique introduced in: https://arxiv.org/abs/2002.04745
    ):
...
class TransformerOutput(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = TransformerLayerNorm(
            config.hidden_size,
            eps=config.layer_norm_eps)  # layer_norm for FFN
...
class TransformerMockingjay(TransformerInitModel):
    ...
    def forward(
        ...
        extended_attention_mask = extended_attention_mask.to(
            dtype=spec_input.dtype)  # fp16 compatibility
        ...
        if head_mask is not None:
            if head_mask.dim() == 1:
                head_mask = (head_mask.unsqueeze(0).unsqueeze(0).unsqueeze(
                    -1).unsqueeze(-1))
                head_mask = head_mask.expand_as(self.config.num_hidden_layers,
                                                -1, -1, -1, -1)
        ...
        if self.with_input_module:
            input_representations = self.input_representations(
                spec_input, pos_enc)
        else:
