...
from minisuperb.util.pseudo_data import get_pseudo_wavs
...
class S3PRLUpstream(nn.Module):
    ...
    def _match_length(self, xs, target_max_len: int):
        ...
        if xs_max_len > target_max_len:
            ...
            xs = torch.cat(
                (xs, xs[:, -1:, :].repeat(1, target_max_len - xs_max_len, 1)),
                dim=1)
    ...
    def forward(self, wavs: torch.FloatTensor, wavs_len: torch.LongTensor):
        ...
        if max(original_wavs_len) < MIN_SECOND * SAMPLE_RATE:
            padded_samples = int(
                MIN_SECOND * SAMPLE_RATE) - max(original_wavs_len)
            wavs = torch.cat(
        ...
        for h, stride in zip(hidden_states, self.downsample_rates):
            ...
            h_len = torch.div(
                original_wavs_len - 1, stride, rounding_mode="floor") + 1
            h = h[:, :max(h_len), :]
            if self.normalize:
...
class Featurizer(nn.Module):
    ...
    def forward(self, all_hs: List[torch.FloatTensor],
        ...
        all_hs = [
            h for idx, h in enumerate(all_hs) if idx in self.layer_selections
        ]
        all_lens = [
            l for idx, l in enumerate(all_lens) if idx in self.layer_selections
        ]
