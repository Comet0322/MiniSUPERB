...
from minisuperb.dataio.encoder.category import CategoryEncoder
from minisuperb.dataio.encoder.g2p import G2P
from minisuperb.dataio.encoder.tokenizer import (
    Tokenizer,
    default_phoneme_tokenizer,
    load_tokenizer,
)
from minisuperb.dataio.encoder.vocabulary import generate_vocab
...
class LoadAudio(DataPipe):
    ...
    def load_audio(
        ...
        wav, sr = torchaudio.load(
            wav_path,
            frame_offset=round(start_sec *
                               self.audio_sample_rate) if crop_segment else 0,
            num_frames=round((end_sec - start_sec) *
                             self.audio_sample_rate) if crop_segment else -1,
        )
    ...
    def forward(self, dataset: AugmentedDynamicItemDataset):
        ...
        if not crop_segment:
            dataset.add_dynamic_item(self.load_audio,
                                     takes=self.wav_path_name,
                                     provides=self.wav_name)
        else:
            dataset.add_dynamic_item(
                self.load_audio,
                takes=[
                    self.wav_path_name, self.start_sec_name, self.end_sec_name
                ],
                provides=self.wav_name,
            )
        dataset.add_dynamic_item(
    ...
    def label_to_binary_vector(label: List, num_labels: int) -> torch.Tensor:
        ...
        if len(label) == 0:
            ...
            binary_labels = torch.zeros(
                (num_labels, )).scatter(0, torch.tensor(label), 1.0)
...
class GenerateTokenizer(DataPipe):
    ...
    def prepare_tokenizer(self, text_list: str = None) -> Tokenizer:
        ...
        if text_list is not None:
            vocab_result = generate_vocab(self.vocab_type,
                                          text_list=text_list,
                                          **vocab_args)
        else:
            vocab_result = generate_vocab(self.vocab_type,
    ...
    def forward(self, dataset: AugmentedDynamicItemDataset):
        try:
            ...
            if self.generate:
                if self.vocab_file is not None and os.path.exists(
                        self.vocab_file):
                    tokenizer = load_tokenizer(
                        self.vocab_type,
                    ...
                    if self.text_file is None:
                        with dataset.output_keys_as([self.text_name]):
                            text_list = [
                                item[self.text_name] for item in dataset
                            ]
...
class EncodeText(DataPipe):
    ...
    def forward(self, dataset: AugmentedDynamicItemDataset):
        try:
            ...
            raise KeyError(
                f"Tokenizer (name = {self.tokenizer_name}) not found!")
...
class RandomCrop(DataPipe):
    ...
    def crop_wav(self, wav):
        if self.max_secs is not None and wav.size(
                0) > self.max_secs * self.sample_rate:
            start = random.randint(
                0,
                wav.size(0) - self.max_secs * self.sample_rate)
