...
class BuildMultiClassTagging(DataPipe):
        ...
        for class_name, start_ends in segments.items():
            ...
            for seg_start, seg_end in start_ends:
                ...
                if rel_seg_start is not None or rel_seg_end is not None:
                    rel_seg_start_frame = (round(rel_seg_start *
                                                 self.sample_rate /
                                                 self.feat_frame_shift) if
                                           rel_seg_start is not None else None)
                    rel_seg_end_frame = (round(rel_seg_end * self.sample_rate /
                                               self.feat_frame_shift)
                                         if rel_seg_end is not None else None)
    ...
    def forward(
        ...
        if not dataset.has_tool(self.all_category_name):
            ...
            dataset.add_tool(self.all_category_name,
                             CategoryEncoder(all_classes))
