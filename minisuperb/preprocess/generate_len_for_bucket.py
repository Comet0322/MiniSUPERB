...
def generate_length(args, tr_set, audio_extension):
    ...
    for i, s in enumerate(tr_set):
        ...
        print('')
        todo = list(
            Path(os.path.join(args.input_data,
                              s)).rglob('*' + audio_extension))  # '*.flac'
        ...
        tr_x = Parallel(n_jobs=args.n_jobs)(delayed(extract_length)(str(file))
                                            for file in tqdm(todo))

        # sort by len
        sorted_todo = [
            os.path.join(s,
                         str(todo[idx]).split(s + '/')[-1])
...
def main():
    ...
    if 'librispeech' in args.input_data.lower():
        SETS = ['librispeech_finetuning', 'dev-clean', 'test-clean']
