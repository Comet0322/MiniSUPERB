...
parser.add_argument('--seed', type=int, default=8830, help='Random seed')
...
def main():
    ...
    with open("{}/clean/utt2spk".format(args.src_dir), 'r') as fh:
        content = fh.readlines()
    uttlist = []
    for line in content:
        line = line.strip('\n')
    ...
    for dset in ["clean", "noisy"]:
        src_dset, tgt_dset = "{}/{}".format(args.src_dir,
                                            dset), "{}/{}".format(
                                                args.tgt_dir, dset)
