...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir,
        ...
        self.objective = SepLoss(self.datarc['num_speakers'], self.loss_type,
    ...
    def forward(self, mode, features, source_attr, uttname_list, source_wav,
        ...
        if mode == 'dev' or mode == 'test':
            if self.log == 'none':
                predict_stfts = [
                    torch.squeeze(m.cpu() * source_attr['stft'])
                    for m in mask_list
                ]
                predict_stfts_np = [
                    np.transpose(s.data.numpy()) for s in predict_stfts
                ]
            elif self.log == 'log1p':
                ...
                predict_stfts = [
                    torch.squeeze(
                        torch.expm1(
                            m.cpu() * torch.log1p(source_attr['stft'].abs())) *
                        phase) for m in mask_list
                ]
                predict_stfts_np = [
                    np.transpose(s.data.numpy()) for s in predict_stfts
                ]
            ...
            predict_srcs_np = [
                librosa.util.fix_length(
                    librosa.istft(stft_mat,
                                  ...
                                  window=self.datarc['window'],
                                  center=self.datarc['center']),
                    size=wav_length[0]) for stft_mat in predict_stfts_np
            ]
            ...
            perm_list = [
                list(perm)
                for perm in list(permutations(range(len(gt_srcs_np))))
            ]
            utt_metrics_list = [
                get_metrics(
                    mix_np,
            ...
            for metric in COMPUTE_METRICS:
                ...
                utt_metrics[metric] = np.max(
                    [k[metric] for k in utt_metrics_list])
    ...
    def log_records(self, mode, records, logger, global_step, batch_ids,
        ...
        if mode == 'train':
            ...
            logger.add_scalar(f"separation_stft2/{mode}-loss",
                              avg_loss,
            ...
            logger.add_scalar(f"separation_stft2/{mode}-loss",
                              avg_loss,
                              global_step=global_step)
            with (Path(self.expdir) /
                  f"{mode}_metrics.txt").open("w") as output:
                for metric in COMPUTE_METRICS:
                    avg_metric = np.mean(records[metric])
                    if mode == "test" or mode == "dev":
                        print("Average {} of {} utts: {:.4f}".format(
                            metric, len(records[metric]), avg_metric))
                        print(metric, avg_metric, file=output)

                    logger.add_scalar(f'separation_stft2/{mode}-' + metric,
                                      avg_metric,
                                      global_step=global_step)
            ...
            for i in range(len(records['uttname'])):
                ...
                for j in range(records['ref'][i].shape[0]):
                    ...
                    hypo_wav = librosa.util.normalize(hypo_wav,
                                                      norm=np.inf,
                                                      axis=None)
                    logger.add_audio('step{:06d}_{}_ref_s{}.wav'.format(
                        global_step, utt, j + 1),
                                     ref_wav,
                                     global_step=global_step,
                                     sample_rate=self.datarc['rate'])
                    logger.add_audio('step{:06d}_{}_hypo_s{}.wav'.format(
                        global_step, utt, j + 1),
                                     hypo_wav,
                                     global_step=global_step,
                                     sample_rate=self.datarc['rate'])
