...
from minisuperb import hub
from minisuperb.optimizers import get_optimizer
from minisuperb.schedulers import get_scheduler
from minisuperb.upstream.interfaces import Featurizer
from minisuperb.utility.helper import is_leader_process, get_model_state, show, defaultdict
...
class Runner():
    ...
    def __init__(self, args, config):
        ...
        self.init_ckpt = torch.load(
            self.args.init_ckpt,
            map_location='cpu') if self.args.init_ckpt else {}

        self.upstream = self._get_upstream()
        self.featurizer = self._get_featurizer()
        self.downstream = self._get_downstream()
        if self.args.use_extracted_feature and args.mode == "train":
            self.all_entries = [self.featurizer, self.downstream]
            del self.upstream
            if "num_workers" in config["downstream_expert"].get('datarc', {}):
                torch.set_num_threads(
                    config["downstream_expert"]['datarc']["num_workers"])
        else:
            self.all_entries = [
                self.upstream, self.featurizer, self.downstream
            ]

        if args.mode == "train" and not args.disable_wandb:
            wandb.init(project=f'fs-superb-{self.args.downstream}',
                       name=args.expname)
    ...
    def _init_model(self, model, name, trainable, interfaces=None):
        ...
        if is_initialized() and trainable and any(
    ...
    def _get_upstream(self):
        ...
        return self._init_model(model=model,
                                name='Upstream',
    ...
    def extract(self):
        ...
        for split in self.args.evaluate_split:
            if os.path.exists(self.args.extracted_path /
                              f"{split}/all_data.ckpt"):
                continue
            # create dir
            os.makedirs(os.path.join(self.args.extracted_path, split),
                        exist_ok=True)
            # create dataloader
            try:
            ...
            # extracted features named after originally name, instead of index
            with torch.no_grad():
                all_data = {}
                print(f"[Runner] - Extract features from {split} split.")
                for i, (ori_wavs, *others) in enumerate(
                        tqdm(dataloader,
                             dynamic_ncols=True,
                             desc=split,
                             file=tqdm_file)):
                    filename = others[1][0]
                    if os.path.exists(
                            self.args.extracted_path /
                            f"extracted_feats/{split}/{filename}.ckpt"):
                        continue
                    wavs = [
                        torch.tensor(wav,
                                     dtype=torch.float,
                    ...
                    selected_features = features.pop(
                        self.args.upstream_feature_selection,
                        features.pop("hidden_states"))
                    ...
                    if self.args.extract_scene_feature:
                        if isinstance(selected_features, (tuple, list)):
                            selected_features = map(time_axis_mean,
                                                    selected_features)
                            selected_features = map(to_cpu, selected_features)
                            selected_features = [
                                data for data in zip(*selected_features)
                            ]
                        else:
                    ...
                    # save feature
                    # if self.args.feature_only:
                    #     torch.save(
                    #                 selected_features, self.args.extracted_path /
                    #                 f"{split}/{filename}.ckpt")
                    # else:
                    for data in zip(selected_features, *others):
                        if self.args.extract_to_single_file:
                            ...
                            torch.save(
                                data, self.args.extracted_path /
                                f"{split}/{filename}.ckpt")
            if self.args.extract_to_single_file:
                torch.save(
                    all_data, self.args.extracted_path /
                    f"extracted_feats/{split}/all_data.ckpt")
    ...
    def train(self):
                    ...
                    if self.args.use_extracted_feature:
                        ...
                        if self.upstream.trainable:
                            ...
                            with torch.no_grad():
                                features = self.upstream.model(wavs)
                    # path = others[1][0]
                    # a = torch.load(f"/media/suisei/DataDisk/DLHLP/data/experiment/asr-fbank-extract-norm/extracted_feats/train/{path}.ckpt", "cpu")[0][0]
                    # torch.save(a[0][0], "temp.ckpt")
                    # b = torch.load("temp.ckpt", "cpu")
                    # print(a[0][0]-features["hidden_states"][0][0][0][0].cpu())
                    # assert False
                    feature_lengths = features.get("feature_lengths",
                                                   feature_lengths)
        ...
        if os.path.exists(os.path.join(self.args.expdir, 'dev-best.ckpt')):
            print(f'[Runner] - Evaluate with dev-best.ckpt')
            self.ckpt = torch.load(os.path.join(self.args.expdir,
                                                'dev-best.ckpt'),
                                   map_location='cpu')
            self.featurizer.model.load_state_dict(self.ckpt['Featurizer'])
            self.downstream.model.load_state_dict(self.ckpt['Downstream'])

        for split in self.config['runner']['test_dataloaders']:
    ...
    def evaluate(self, split=None, logger=None, global_step=0):
        ...
        if not_during_training:
            assert len(
                self.args.evaluate_split
            ) == 1, "Not support evaluating two splits at the same run."
        ...
        # prepare data
        dataloader = self.downstream.model.get_dataloader(split)
        evaluate_ratio = float(
            self.config.get("runner", {}).get("evaluate_ratio", 1))
            ...
            with torch.no_grad():
                if self.args.use_extracted_feature:
                    ...
                    wavs = [
                        torch.tensor(wav,
                                     dtype=torch.float,
                                     device=self.args.device) for wav in wavs
                    ]
                    features = self.upstream.model(wavs)
    ...
    def push_to_huggingface_hub(self):
        ...
        if self.args.hub == "huggingface":
            model_info = HfApi().model_info(self.args.upstream,
        ...
        model_repo = Repository(local_dir=REPO_ROOT_DIR,
                                clone_from=repo_url,
        ...
        shutil.copytree(self.args.expdir,
                        REPO_TASK_DIR,
                        dirs_exist_ok=True,
        ...
        if len(checkpoints) == 0:
            ...
            CKPT_PATH = (os.path.join(
                REPO_TASK_DIR,
                f"states-{self.config['runner']['total_steps']}.ckpt"))
