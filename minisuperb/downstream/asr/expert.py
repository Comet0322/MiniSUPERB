...
from minisuperb.utility.data import get_extracted_asr_dataset
...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir,
        ...
        self.dictionary = Dictionary.load(
            self.datarc.get("dict_path",
                            str(Path(__file__).parent / "char.dict")))
        ...
        if kwargs["use_extracted_feature"] and kwargs["mode"] != "extract":
            ...
            dataset_cls = get_extracted_asr_dataset(
                SequenceDataset,
                kwargs['extract_to_single_file'],
            )
        else:
            dataset_cls = SequenceDataset
        self.dataset_cls_ger = lambda split, batch_size: dataset_cls(
            split,
            batch_size,
            self.dictionary,
            **self.datarc,
        ...
        self.objective = nn.CTCLoss(blank=self.blank,
        ...
        if not hasattr(self, f'{split}_dataset'):
            if not batch_size:
                batch_size = self.datarc[
                    'train_batch_size'] if split == "train" else self.datarc[
                        'eval_batch_size']
            setattr(self, f'{split}_dataset',
                    self.dataset_cls_ger(split, batch_size))
    ...
    def _compute_metrics(self, pred_tokens_all, pred_words_all,
                         target_tokens_all, target_words_all):
        ...
        for pred_tokens, pred_words, target_tokens, target_words in zip(
                pred_tokens_all, pred_words_all, target_tokens_all,
                target_words_all):
    ...
    def _decode(self, log_probs, input_lens):
        ...
        for log_prob, in_len in zip(log_probs, input_lens):
            ...
            pred_token_ids = pred_token_ids[pred_token_ids !=
                                            self.blank].tolist()
    ...
    def forward(self, split, features, labels, filenames, records, **kwargs):
        ...
        labels_len = torch.IntTensor([len(label)
                                      for label in labels]).to(device=device)
        labels = pad_sequence(
            labels,
            batch_first=True,
    ...
    # interface
    def log_records(self, split, records, logger, global_step, batch_ids,
        ...
        if 'test' in split or 'dev' in split:
            ...
            hyp_ark = open(os.path.join(self.expdir, f'{split}-{lm}-hyp.ark'),
                           'w')
            ref_ark = open(os.path.join(self.expdir, f'{split}-{lm}-ref.ark'),
                           'w')
            for filename, hyp, ref in zip(records['filenames'],
                                          records['pred_words'],
                                          records['target_words']):
                hyp = ' '.join(hyp)
                ref = ' '.join(ref)
