...
from minisuperb.utility.helper import show
...
class UpstreamBase(nn.Module, metaclass=initHook):
    ...
    def __init__(
        ...
        hook_postprocess: Callable[[List[Tuple[str, Tensor]]],
    ...
    def __call__(self, wavs: List[Tensor], *args, **kwargs):
        ...
        if len(self._hook_hiddens) > 0:
            if (result.get("_hidden_states_info") is not None
...
class Featurizer(nn.Module):
        ...
        **kwargs,
        ...
        if isinstance(feature, (list, tuple)):
            ...
            self.layer_norm = nn.LayerNorm(
                (feature[0].size(-1), self.layer_num))
    ...
    def _select_feature(self, features):
        ...
        if isinstance(feature,
                      (list, tuple)) and isinstance(self.layer_selection, int):
    ...
    def _weighted_sum(self, feature):
        ...
        if self.normalize:
            stacked_feature = self.layer_norm(stacked_feature)

        stacked_feature = stacked_feature.permute(3, 0, 1, 2)

        _, *origin_shape = stacked_feature.shape
        stacked_feature = stacked_feature.contiguous().view(self.layer_num, -1)
    ...
    def tolist(self,
        ...
        if cif_lengths:
            ...
            b = round(
                max([len(wav) for wav in paired_wavs]) / self.downsample_rate)
            ...
            feature_len = [
                round(len(wav) / self.downsample_rate) for wav in paired_wavs
            ]
    ...
    def forward(self,
                paired_wavs: List[Tensor],
                paired_features: Dict[str, Union[Tensor, List[Tensor],
                                                 Dict[str, Tensor]]],
                cif_lengths=None):
        feature = self._select_feature(paired_features)
