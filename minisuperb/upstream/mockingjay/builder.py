...
class TransformerBuilder(nn.Module):
    ...
    def __init__(self,
        ...
        self.output_hidden_states = bool(
            strtobool(options["output_hidden_states"]))
        ...
        if on_the_fly_config is not None:
            self.config["audio"] = yaml.load(open(on_the_fly_config, "r"),
                                             Loader=yaml.FullLoader)
        if "audio" in self.config:
            if "kaldi" in self.config["audio"]:
                self.extracter, self.inp_dim, _ = get_extracter(
                    self.config["audio"])
        ...
        try:
            ...
            if len(missing_keys) > 0:
                print(
                    ...
                    format(transformer_model.__class__.__name__, missing_keys))
            ...
            raise RuntimeError(
                "[Transformer] - Pre-trained weights NOT loaded!")
    ...
    def process_input_data(self, feat):
        ...
        # input `feat` should have shape BxTxD
            ...
            raise ValueError(
                "Input argument `feat` has invalid shape: {}".format(
                    feat.shape))
        ...
        pos_enc = position_encoding(seq_len,
                                    self.hidden_size)  # (seq_len, hidden_size)
        ...
        if (self.spec_aug and self.spec_aug_prev and self.model.training
                and self.inp_dim > 1):
            feat = spec_augment(
                feat, mask_T=70, mask_F=9, num_T=2, num_F=2,
                p=1.0)  # (batch_size, seq_len, feature_dim * dr)
        feat = feat.to(
            dtype=torch.float32)  # (batch_size, seq_len, feature_dim * dr)
        pos_enc = (torch.FloatTensor(pos_enc).to(device=feat.device,
                                                 dtype=torch.float32).expand(
                                                     feat.size(0),
                                                     *pos_enc.size())
                   )  # (batch_size, seq_len, hidden_size)
        attn_mask = torch.FloatTensor(attn_mask).to(
            device=feat.device, dtype=torch.float32)  # (batch_size, seq_len)
    ...
    def _forward(self, x):
        ...
        # forward the sequence in chunks then concat
            ...
            chunks = torch.chunk(x,
                                 chunks=math.ceil(input_len /
                                                  self.max_input_length),
                                 dim=1)
            x_ = []
            for chunk in chunks:
            ...
            x = torch.cat(
                x_,
                dim=2 if
                (self.output_hidden_states or self.select_layer != -1) else 1,
            )
        ...
        if (self.spec_aug and not self.spec_aug_prev and self.model.training
                and self.inp_dim > 1):
            x = spec_augment(x, mask_T=70, mask_F=86, num_T=2, num_F=2,
                             p=1.0)  # (B, T, D)
...
class PretrainedTransformer(TransformerBuilder):
    ...
    def __init__(self,
                 ...
                 online_config=None,
                 verbose=False):
        super(PretrainedTransformer, self).__init__(options, inp_dim, config,
                                                    online_config, verbose)
    ...
    def forward(self, x):
        if self.extracter is not None:
            if "kaldi" in self.config["audio"]:
                ...
                x = pad_sequence(x, batch_first=True)
                x = x.unsqueeze(
                    1)  # (batch_size, audio_len) -> (batch_size, 1, audio_len)
...
class PretrainedTransformerWithHead(PretrainedTransformer):
    ...
    def __init__(self,
                 ...
                 online_config=None,
                 verbose=False):
        super(PretrainedTransformerWithHead,
              self).__init__(options, inp_dim, config, online_config, verbose)
        ...
        if self.load:
            ...
            if verbose:
                print("[Spec Transformer] - Number of parameters: " + str(
                    sum(p.numel() for p in self.SpecHead.parameters()
                        if p.requires_grad)))
        ...
        for idx in range(spec.shape[0]):
            ...
            if mask_T > 0 and mask_T < upper_bound:
                for _ in range(num_T):
                    ...
                    chosen_start = torch.randperm(spec.shape[1] -
                                                  rand_consecutive)[:1]
            ...
            if mask_F > 0:
                for _ in range(num_F):
                    ...
                    chosen_start = torch.randperm(spec.shape[2] -
                                                  rand_bandwidth)[:1]
                    chosen_intervals = _start_to_intervals(
                        chosen_start, rand_bandwidth)
                    spec[idx, :, chosen_intervals] = 0
