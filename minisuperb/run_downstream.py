...
from minisuperb.downstream.runner import Runner
from minisuperb.utility.helper import backup, get_time_tag, hack_isinstance, is_leader_process, override
...
def get_downstream_args():
    ...
    parser.add_argument('-m',
                        '--mode',
                        choices=['train', 'evaluate', 'inference', 'extract'],
                        required=True)
    ...
    parser.add_argument('-v',
                        '--downstream_variant',
                        help='Downstream vairants given the same expert')
    ...
    parser.add_argument(
        '--upstream_model_name',
        default="model.pt",
        help='The name of the model file in the HuggingFace Hub repo.')
    parser.add_argument(
        '--upstream_revision',
        help="The commit hash of the specified HuggingFace Repository")

    # experiment directory, choose one to specify
    # expname uses the default root directory: result/downstream
    parser.add_argument('-n',
                        '--expname',
                        help='Save experiment at result/downstream/expname')
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        'Push all files in experiment directory to the Hugging Face Hub. To use this feature you must set HF_USERNAME and HF_PASSWORD as environment variables in your shell'
    )
    parser.add_argument(
        '--hf_hub_org',
        help='The Hugging Face Hub organisation to push fine-tuned models to')
    ...
    parser.add_argument('--verbose',
                        action='store_true',
                        help='Print model infomation')
    parser.add_argument('--disable_cudnn',
                        action='store_true',
                        help='Disable CUDNN')
    ...
    # extract mode
    parser.add_argument('--use_extracted_feature',
                        action='store_true',
                        help='Extract features first, then train.')
    parser.add_argument('--extracted_path',
                        help="The path to put 'extracted_feats/'")
    parser.add_argument('--extract_scene_feature',
                        action='store_true',
                        help='Aggregate feature along time axis. ')
    parser.add_argument(
        '--scene_method',
        choices=["mean"],
        default="mean",
        help="The method to aggregate feature along time axis.")
    parser.add_argument(
        '--extract_to_single_file',
        action='store_true',
        help=
        'This mode will save features to a single file and load it to memory during training.'
    )
    ...
    # extract mode
    assert ((not args.use_extracted_feature or args.downstream
             in ("asr", "voxceleb1", "enhancement_stft2", "separation_stft2")))
    if args.mode == "extract" and args.evaluate_split == "all":
        args.evaluate_split = (config['runner'].get("train_dataloader",
                                                    "train"),
                               *config['runner']['eval_dataloaders'],
                               *config['runner']['test_dataloaders'])
    else:
        args.evaluate_split = re.split(r"[, ]+", args.evaluate_split)
    if args.use_extracted_feature and args.mode == "train":
        print('[Runner] - Training downstream model with extracted features')
...
def main():
    ...
    if args.hub == "huggingface":
        ...
        huggingface_token = HfApi().login(username=hf_user,
