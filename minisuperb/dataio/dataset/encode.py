...
from minisuperb.dataio.encoder.category import CategoryEncoder, CategoryEncoders
from minisuperb.dataio.encoder.tokenizer import Tokenizer
...
class EncodeCategories(Dataset):
    ...
    def __init__(self, labels: List[List[str]],
                 encoders: CategoryEncoders) -> None:
        super().__init__()
        self.labels = labels
...
class EncodeMultiLabel(Dataset):
    ...
    @staticmethod
    def label_to_binary_vector(label_ids: List[int],
                               num_labels: int) -> torch.Tensor:
        if len(label_ids) == 0:
            binary_labels = torch.zeros((num_labels, ), dtype=torch.float)
        else:
            binary_labels = torch.zeros(
                (num_labels, )).scatter(0, torch.tensor(label_ids), 1.0)

        assert set(
            torch.where(binary_labels == 1.0)[0].numpy()) == set(label_ids)
        return binary_labels

    def __getitem__(self, index: int):
        labels = self.labels[index]
        label_ids = [self.encoder.encode(label) for label in labels]
        binary_labels = self.label_to_binary_vector(label_ids,
                                                    len(self.encoder))
