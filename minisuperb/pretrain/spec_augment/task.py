...
import torch
import random
from minisuperb.pretrain.mockingjay.task import fast_position_encoding
...
def generate_spec_aug_data(spec, config):
    ...
    with torch.no_grad():
        ...
        if len(
            ...
            spec_target = copy.deepcopy(
                spec[0])  # (batch_size, seq_len, feat_dim)
        else:
            raise ValueError
        ...
        pos_enc = fast_position_encoding(seq_len,
        ...
        for idx in range(batch_size):
            ...
            def _starts_to_intervals(starts, consecutive):
                tiled = starts.expand(consecutive,
                                      starts.size(0)).permute(1, 0)
                offset = torch.arange(consecutive).expand_as(tiled)
                intervals = tiled + offset
                return intervals.view(-1)

            # time masking
            upper_bound = spec_len[idx] * config[
                'p']  # upper bound on the time mask so that a time mask cannot be wider than p times the number of time steps
            if config['mask_T'] > 0 and config['mask_T'] < upper_bound:
                for _ in range(config['num_T']):
                    ...
                    chosen_start = torch.randperm(spec_masked.shape[1] -
                                                  rand_consecutive)[:1]
                    chosen_intervals = _starts_to_intervals(
            ...
            if config['mask_F'] > 0:
                for _ in range(config['num_F']):
                    ...
                    chosen_start = torch.randperm(spec_masked.shape[2] -
                                                  rand_bandwidth)[:1]
                    chosen_intervals = _starts_to_intervals(
        ...
        valid_batchid = mask_label.view(
            batch_size, -1).sum(dim=-1).nonzero(as_tuple=False).view(-1)
